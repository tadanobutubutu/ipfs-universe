...
- 1,024件を描画し続けても操作を止めないため、ZigのO(n²)反発計算は品質に応じた近傍予算（デスクトップ最大256、モバイル最大96）だけを毎フレーム更新します。予算外の外周ノードは実測半径と決定的な初期位置を保ち、表示数・距離・選択可能性を削りません。
- ピア数が増えても自動フレーミングはワールド距離150を上限（縦長は145）にし、中心のホスト星が小さく潰れないまま外周ピアを視認・選択できます。手動のWASD／矢印・ホイール移動は180まで使えます。
- ピアのワールド配置はカメラの上限とは別です。測定できたpingを対数スケールで距離へ反映し、高遅延・遠いピアはカメラの外側へ残します（位置の数値安全上限は2,048で、通常の観測値はそこへ到達しません）。自動カメラは150（縦長145）で止まるため、密な観測でも中心星の存在感を失わず、左ドラッグ、右クリック切替で外周を探索できます。
- 自動フレーミングは、ブラウザliveとping実測済みの観測を優先し、未計測Kuboの大量インポートでカメラを引きません。ブラウザliveがまだ無い初回だけ、決定的に近い24件を導入点として使い、残りの遠いノードは同じ世界座標のまま外周へ残します。
- 右上の歯車から、最低〜最高のプリセットまたは自動品質を選べます。ピクセル比、アンチエイリアス、星雲・星屑密度、ノード詳細度、線の明るさ、露出、深度フォグ、モーション、カメラ感度、表示ソース、到着パルスを個別に調整できます。設定は端末内だけに保存され、初期値は自動品質です。スカイボックスはライセンスとSHA256を確認した素材だけをクレジット画面へ追加します。
...
3Dキャンバスがフォーカスされている時、`W`／`S` は前後移動、`A`／`D` と左右矢印は旋回、上下矢印は俯仰です。左ボタンを押している間はカメラを回し、右クリックでカメラモードを固定できます。右クリックまたは `Escape` でホバーモードへ戻ると、ポインターをノードへ重ねた時にそのノードから詳細カードが伸びます。スマートフォンではスワイプ・ピンチ・タップを使います。

歯車の設定は、画質（自動、最低、低、中、高、最高）、描画密度、空気遠近、モーション、入力感度、Kubo／発見ピアの表示、レイテンシリングを同じダイアログで調整します。自動モードだけが実測フレームp95に応じて降格・回復し、手動プリセットは選択値を勝手に変更しません。設定ダイアログのクレジット欄には、採用した宇宙パノラマの作者・出所・ライセンス・解像度・ハッシュを掲載します。
