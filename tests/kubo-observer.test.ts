...
describe('probeLocalKubo', () => {
  ...
  it('maps verbose Kubo peers into typed observations without exposing addresses', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ ID: 'local' }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ Peers: [{
        Peer: '12D3KooWpeer',
    ...
    expect(result.observations).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: 'connected', source: 'kubo', direction: 'outbound', transport: 'quic-v1', agentVersion: 'kubo/0.41.0/', addressCount: 1 }),
